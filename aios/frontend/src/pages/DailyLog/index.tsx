    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between px-1">
        <button onClick={() => { if (month === 1) { setYear(y => y - 1); setMonth(12) } else setMonth(m => m - 1) }}
          className="size-7 flex items-center justify-center rounded hover:bg-muted text-muted-foreground">‹</button>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/20">
      <div className="bg-card border border-border rounded-xl shadow-lg w-full max-w-sm mx-4 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="size-6 flex items-center justify-center rounded hover:bg-muted text-muted-foreground text-lg">×</button>
        </div>
        <div className="flex flex-col gap-2 mb-5">
          {habits.map(h => (
            <div key={h.id} className="flex items-center justify-between py-1">
              ...
              >
                <div className={`absolute top-0.5 size-3 rounded-full bg-white transition-transform duration-150 ${h.active ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </button>
            </div>
          ))}
      ...
      <div className="hidden md:flex w-[360px] shrink-0 border-r border-border flex-col overflow-hidden">
        <div className="flex items-center justify-between px-4 pt-4 pb-3 shrink-0">
          ...
          <button
            ...
            className="size-7 flex items-center justify-center rounded hover:bg-muted text-muted-foreground"
            title="Manage habits"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="md:hidden flex items-center gap-2 px-3 py-2.5 border-b border-border shrink-0">
          ...
          <button
            ...
            className="size-7 flex items-center justify-center rounded hover:bg-muted text-muted-foreground disabled:opacity-30"
          ><ChevronRight size={16} /></button>
          <button
            onClick={() => setShowHabitManager(true)}
            className="size-7 flex items-center justify-center rounded hover:bg-muted text-muted-foreground ml-1"
          ><Settings size={14} /></button>
        </div>
