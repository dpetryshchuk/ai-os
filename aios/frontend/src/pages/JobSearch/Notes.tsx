...
function NoteRow({ note, onUpdate, onDelete }: {
  ...
  const saveEdit = async () => {
    setSaving(true)
    try {
      const r = await fetch(`/api/jobsearch/notes/${note.id}`, {
        method: 'PATCH',
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    await fetch(`/api/jobsearch/notes/${note.id}`, { method: 'DELETE' })
    onDelete(note.id)
  }
...
export default function Notes() {
  ...
  const save = async () => {
    ...
    setSaveError(null)
    try {
      const r = await fetch('/api/jobsearch/notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: fCategory,
