...
export default function Archive({ days, habits, selectedDate, onSelectDate }) {
...
function YearGroup({ year, months, activeHabits, selectedDate, onSelectDate }) {
  ...
  return (
    <div>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full text-left flex items-center gap-1.5 py-1.5 text-[12px] font-medium text-foreground hover:text-foreground"
      >
        <span className="text-muted-foreground w-3 text-[10px]">{open ? 'â–¾' : 'â–¸'}</span>
        {year}
...
function MonthGroup({ month, year, days, activeHabits, selectedDate, onSelectDate }) {
  ...
  return (
    <div>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full text-left flex items-center gap-1.5 py-1 text-[12px] text-muted-foreground hover:text-foreground"
      >
        <span className="w-3 text-[10px]">{open ? 'â–¾' : 'â–¸'}</span>
        {monthName}
        ...
        <div className="ml-3 border-l border-border">
          {dayKeys.map(day => (
            <DayRow
              ...
              day={days[day]}
              activeHabits={activeHabits}
              isSelected={days[day].date === selectedDate}
              onSelect={() => onSelectDate(days[day].date)}
            />
          ))}
        </div>
      )}
...
function DayRow({ day, activeHabits, isSelected, onSelect }) {
  const [open, setOpen] = useState(false)
  ...
  return (
    <div className={`pl-3 mb-px ${isSelected ? 'bg-muted rounded' : ''}`}>
      <div className="flex items-center gap-1">
        <button
          onClick={() => setOpen(o => !o)}
          className="text-[10px] text-muted-foreground w-3 py-1 flex-shrink-0"
        >
          {open ? 'â–¾' : 'â–¸'}
        </button>
        <button
          onClick={onSelect}
          className="flex-1 text-left flex items-center gap-2 py-1 text-[12px] text-foreground hover:text-foreground"
        >
          <span>{label}</span>
